...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        r = (torch.rand(size=(1,)) < 0.1).item() is False

        return x, dict(
            c_crossattn_ctrl=[c_ctrl if r else torch.zeros_like(c_ctrl)],
            c_crossattn_diff=[c_diff if r else torch.zeros_like(c_diff)],
            c_concat=[control],
        )
